 ...
 */
export function generateFingerprintId(clientInfo: Record<string, unknown>): {
  id: string;
  confidence: number;
} {
  ...
  private calculateHandedness(): {
    ...
    if (this.state.mousePositions.length < 50) {
      return {
        hand: "unknown",
        confidence: 0,
        warning: "Insufficient mouse data for reliable handedness detection",
      };
    }
    ...
    const warning =
      "Low confidence: mouse position is heavily influenced by UI layout, not handedness";

    if (rightRatio > 0.6) {
      return {
        hand: "right",
        confidence: Math.round(rightRatio * 100),
        warning,
      };
...
function parseGPU(renderer: string | null | undefined): {
  ...
  if (r.includes("nvidia") || r.includes("geforce") || r.includes("rtx") || r.includes("gtx")) {
    ...
    if (r.includes("1650") || r.includes("1660"))
      return {
        brand: "NVIDIA",
        model: "GTX 16xx",
        tier: "entry",
...
export function generateUserProfile(clientInfo: Partial<ClientInfo>): UserProfile {
  ...
  const usedDevShortcuts =
    clientInfo.advancedBehavior?.keyboardShortcutsUsed?.filter((k) =>
      devShortcuts.some((d) => k.toUpperCase().includes(d)),
    ) || [];
  ...
  if (
    workstationGPU.includes("quadro") ||
    workstationGPU.includes("firepro") ||
    workstationGPU.includes("radeon pro")
  ) {
    designerScore += 45;
  ...
  if (
    ...
    !clientInfo.hardwareFamily?.includes("Tablet")
  ) {
    designerScore += 12;
  ...
  const advancedShortcuts = shortcutsUsed.filter(
    (s) => s.includes("SHIFT") || s.includes("ALT") || s.includes("OPT"),
  );
  ...
  const foundPowerExtensions =
    clientInfo.extensionsDetected?.filter((e) =>
