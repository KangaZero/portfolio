...
import { Flex, Grid, Row, StatusIndicator, Tag, ToggleButton } from "@once-ui-system/core";
...
export default function AchievementsWrapper() {
  ...
  const [currentSelectedFilters, setCurrentSelectedFilters] = useState<Achievement["rarity"][]>([]);
  ...
  const isAllVisibleAchievementsUnlocked =
    achievements.filter(
      (achievement) => achievement.isUnlocked && achievement.title !== "Speedophile",
  ...
  const filteredAchievements = achievementsToDisplay.filter(
    (achievement) =>
      achievement.title.toLowerCase().includes(currentSearchTerm.toLowerCase().trim()) &&
      (currentSelectedFilters.length === 0 || currentSelectedFilters.includes(achievement.rarity)),
  );
  const searchResultDescription = filteredAchievements
    ? `${filteredAchievements.length} result(s) found.`
    : "No results found.";

  const removeFilter = (filter: Achievement["rarity"], e: PointerEvent<HTMLDivElement>) => {
    ...
    gsap.to(element, {
      ...
      ease: "power1.inOut",
      onComplete: () =>
        setCurrentSelectedFilters(currentSelectedFilters.filter((f) => f !== filter)),
    });
  };
  ...
  const toggleColumns = () => {
    ...
    const currentColumns = getComputedStyle(achievementsGridRef.current).gridTemplateColumns;
    ...
    setStatusIndicatorColor(columnCount === 1 ? "aqua" : columnCount === 2 ? "red" : "emerald");
    Flip.from(state, {
      duration: 0.5,
      ease: "power1.inOut",
      stagger: 0.02,
